...
- **emotional_echo**：情感检测从「关键词匹配」升级为「LLM 智能判断制」
  - LLM 优先判断 + 关键词兜底，支持反讽、否定句、网感语境
  - 情绪集合扩展：新增 `anxious` / `fear`（归入负向 holding 频道）
  - 置信度阈值：LLM 低置信不记峰值、不打扰；`neutral` 自然回归
  - 新增文本哈希缓存（300 条）省 LLM API 调用
  - 配置新增 `llm_emotion_enabled` 开关（默认开启）
  - 真实场景 6 项实测全通过（关键词兜底 / 高置信记峰值 / 低置信不打扰 / neutral 回归 / anxious 归负向 / 纯关键词）
